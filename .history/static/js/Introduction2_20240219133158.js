    ...
    saveButton.addEventListener('click', function() {
        ...
        if (file) {
            ...
            reader.onloadend = function() {
                ...
                // 既存のimgタグを取得するか、存在しない場合は新しいimgタグを作成
                let img = saveArea.querySelector('img');
                if (!img) {
                    img = document.createElement('img');
                    img.style.maxWidth = '100%';
                    img.style.maxHeight = '100%';
                    img.style.minWidth = '100%'; // 必要に応じて調整
                    img.style.minHeight = '100%'; // 必要に応じて調整
                    saveArea.appendChild(img);
    }

                // 背景削除後の画像URLを使用して、imgタグのsrc属性を更新
