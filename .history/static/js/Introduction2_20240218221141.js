document.addEventListener('DOMContentLoaded', function() {
    ...
    backgroundTransparencyButton.addEventListener('click', function() {
        if (isImageUploaded) {
            ...
            .then(response => {
                if (response.ok) {
                    return response.json(); // JSONオブジェクトに変換
                }
                throw new Error('Network response was not ok.');
            })
            .then(data => {
                // JSONレスポンスから画像のURLを取得
                const imageUrl = data.file_path; // Flaskからのレスポンスに含まれるファイルパス
                const preview = document.getElementById('background_transparency');
                preview.src = imageUrl; // 画像URLをimgタグのsrcに設定
                document.getElementById('background_transparency').style.display = 'block'; // コンテナを表示
            })
