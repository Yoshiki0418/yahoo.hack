...
function setupAddItemButton() {
    ...
    if (addItemButton) {
        ...
        addItemButton.addEventListener2('click', addItem);
...
// モーダル関連のイベントリスナーを設定
document.addEventListener('DOMContentLoaded', function() {
    document.getElementById('myBtn').addEventListener2('click', openModal);
    document.getElementsByClassName('close')[0].addEventListener2('click', closeModal);
    document.addEventListener2('click', function(event) {
      if (event.target == document.getElementById('myModal')) {
        closeModal();
      }
    });
