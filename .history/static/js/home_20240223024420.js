...
document.getElementById('nextCoordination').addEventListener('click', function() {
  // 現在表示中のコーディネートを隠すロジックをここに追加
  // 次に表示するコーディネートを表示するロジックをここに追加
  
  // 例: 簡単な実装として、全てのコーディネートを配列に格納し、
  // インデックスを使って順番に表示する方法
  var coordinations = document.querySelectorAll('.movie_container_main');
  var currentIndex = 0; // 現在のコーディネートのインデックス

  // 全てのコーディネートを非表示にする
  coordinations.forEach(function(coordination) {
      coordination.style.display = 'none';
  });

  // 次のコーディネートを表示
  currentIndex = (currentIndex + 1) % coordinations.length; // 次のインデックスを計算
