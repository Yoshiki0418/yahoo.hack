...
function displayImage11(event, containerId) {
    ...
    if (file) {
        ...
        reader.onload = function(e) {
            ...
            newImgContainer.className = 'add_item_pic';

            // 新しい画像要素を作成
            var newImg = document.createElement('img');
            newImg.src = e.target.result; // ファイルリーダーの結果を画像のsrcに設定

            // 画像コンテナに画像要素を追加
            newImgContainer.appendChild(newImg);

            // アイテムブロックに画像コンテナを追加
            newItemBlock.appendChild(newImgContainer);

            // 最終的に、アイテムブロックをページの指定された位置に追加
            document.getElementById(containerId).appendChild(newItemBlock);
        };
