...
document.addEventListener("DOMContentLoaded", function() {
  ...
  document.addEventListener('scroll', function() {
    var triggerHeight = 900;
    var pageOffset = window.pageYOffset || document.documentElement.scrollTop;
    
    if (pageOffset > triggerHeight) {
      var textSpans = document.querySelectorAll('.point_ja span, .point_en span');
      textSpans.forEach(function(span, index) {
        // アニメーションを遅延させて、一文字ずつ順に色を変える
        span.style.animationDelay = `${index * 0.1}s`;
        span.style.animation = 'text-color-animation 0.3s forwards';
      });
    }
  });
