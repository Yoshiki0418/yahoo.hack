...
def home():
    if "usr" in session:
        ...
        post_closet_groups = {}
        for style in similarStyle:
            post_closets_for_style = PsotCloset.query.filter(PsotCloset.style_id.in_([style.id for style in similarStyle])).all()
            post_closet_groups[style.id] = post_closets_for_style
