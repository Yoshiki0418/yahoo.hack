...
def home():
    if "usr" in session:
        ...
        for post in posts:
            ...
            post_details.append({
                ...
                'items': [{
                    'image': pc.image,
                    'price': pc.price,
                    'style': pc.style.name if pc.style else '',
                    'category': pc.category,  # 仮のプロパティ、実際のモデルに合わせてください
                    'brand': pc.brand  # 仮のプロパティ、実際のモデルに合わせてください
        ...
        return render_template('home.html', closet=mycloset,data=post_closet_groups)
    else:
        return render_template('welcome.html')
